...
BOOST_AUTO_TEST_CASE(TC1_zeros, *utf::tolerance(0.00001)) {
...
BOOST_AUTO_TEST_CASE(TC5_largeNumbers, *utf::tolerance(0.00001)) {
...
BOOST_AUTO_TEST_CASE(TC1_zeros) {
    ...
    auto original = left;
    memcpy(&original.Elements, &left.Elements, 16 * sizeof(int));
    left -= right;
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
