...
BOOST_AUTO_TEST_CASE(TC1_zeros) {
    ...
    auto res = HMM_AddVec3(left, right);
    BOOST_TEST(res.X == 0);
    BOOST_TEST(res.Y == 0);
    BOOST_TEST(res.Z == 0);
...
BOOST_AUTO_TEST_CASE(TC1_zeros) {
    ...
    auto res = HMM_AddVec4(left, right);
    BOOST_TEST(res.X == 0);
    BOOST_TEST(res.Y == 0);
    BOOST_TEST(res.Z == 0);
    BOOST_TEST(res.W == 0);
...
BOOST_AUTO_TEST_CASE(TC1_zeros) {
    ...
    auto res = HMM_AddMat4(left, right);
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
